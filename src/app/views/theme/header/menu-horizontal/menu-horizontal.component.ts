...
export class MenuHorizontalComponent implements OnInit, AfterViewInit {
	...
	ngOnInit(): void {
		...
		this.menuHorService.menuList$.subscribe(items => {
			if (items.length > 0) {
				let first = items[0];
				this.storage.setActiveMenu(first.id);
				console.log("Loaded First : ", first.id);
				this.subheader.loadMenus();
	...
	mouseClick(item) {
		...
		this.storage.setActiveMenu(item.id);
		this.subheader.loadMenus();
		this.cdr.detectChanges();
	}
